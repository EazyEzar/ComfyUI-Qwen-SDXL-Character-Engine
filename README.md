...
![Banner](assets/final_result_test.png)
...
We utilize a hard-edged 3-panel mask (`assets/template_3box.png`) to constrain Qwen. This forces the model to generate three distinct views of the *same* entity in one pass, ensuring outfit and facial consistency.

| Input Template | Generated Dataset (Qwen) |
| :---: | :---: |
| ![Template](assets/template_3box.png) | ![Dataset](assets/generated_dataset.png) |
...
**The Fix:** When using an overfit LoRA, we lower the `strength_model` during inference (e.g., **0.76** instead of 1.0). This recovers the flexibility while keeping the likeness, as seen in the final results.
...
### Steps
1.  **Setup:** Place `template_3box.png` in your ComfyUI input folder.
2.  **Generate & Train:**
    * Load `workflows/01_generate_and_train.json`.
    * Set your prompt in the Qwen text node.
    * Click Queue. (Approx. time: 10-15 mins).
3.  **Inference:**
    * Load `workflows/02_inference_test.json`.
